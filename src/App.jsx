    ...
    <div className={`fixed inset-0 text-white font-sans overflow-hidden transition-colors duration-500 ${transparent ? 'bg-transparent' : 'bg-black'}`}>
        ...
        {view === 'draw' && (
          <div className="flex items-center gap-5">
            ...
            <div className="relative">
                <BrainCircuit size={24} onClick={analyzeDrawing} className={`${isAnalyzing ? 'animate-pulse text-purple-400' : 'text-purple-400'} cursor-pointer`} />
                {analysisResult && (
            ...
            <Undo size={24} onClick={() => history.current.length && ctxRef.current.putImageData(history.current.pop(), 0, 0)} className="text-zinc-400 active:scale-90 cursor-pointer" />
            <Trash2 size={24} onClick={() => ctxRef.current.clearRect(0, 0, canvasRef.current.width, canvasRef.current.height)} className="text-red-500 active:scale-90 cursor-pointer" />
            <Save size={24} onClick={() => setShowSaveModal(true)} className="text-green-500 active:scale-90 cursor-pointer" />
          </div>
        )}
      </div>
      ...
      {view !== 'draw' && (
        <div className="absolute inset-0 z-[60] bg-black overflow-y-auto pt-32 px-6 pb-24">
          <div className="flex justify-between items-center mb-8"><h1 className="text-3xl font-black uppercase tracking-tighter italic">{view}</h1><X size={32} onClick={() => setView('draw')} className="text-zinc-500 cursor-pointer" /></div>
          {view === 'gallery' && (
            <div className="grid grid-cols-2 gap-4">
              {galleryItems.length === 0 ? <p className="col-span-2 text-center text-zinc-500 text-xs">The void is empty.</p> : galleryItems.map(item => (
                <div key={item.id} className="aspect-square rounded-3xl overflow-hidden bg-zinc-900 border border-white/5 shadow-xl"><img src={item.image} alt="Gallery creation" className="w-full h-full object-cover" /></div>
              ))}
            </div>
          )}
          {view === 'profile' && (
